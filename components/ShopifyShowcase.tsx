         ...
         <div className="section-shell">
            ...
            <div className="card-grid md:grid-cols-3">
               {projects.map((project, i) => (
                  <div
                     key={project.id}
                     className="group bg-[#0a0a0a] rounded-lg md:rounded-3xl overflow-hidden border border-white/5 hover:border-white/10 transition-all duration-500 hover:shadow-2xl"
                  >
                     {/* Reduced Thumbnail Size ONLY */}
                     <div className="relative h-32 md:h-40 lg:h-48 overflow-hidden">
                        ...
                        <div className="absolute bottom-3 md:bottom-6 left-3 md:left-6 right-3 md:right-6">
                           <h3 className="text-lg md:text-2xl font-bold text-white mb-1 md:mb-2 line-clamp-1">
                              {project.title}
                           </h3>
                           <div className="flex flex-wrap gap-1.5 md:gap-2">
                              {project.technologies.slice(0, 2).map((tech) => (
                                 <span
                                    key={tech}
                                    className="text-[9px] md:text-[10px] bg-white/10 backdrop-blur-md px-2 py-0.5 md:py-1 rounded-md text-white font-medium"
                                 >
                                    {tech}
                                 </span>
                              ))}
                     ...
                     <div className="p-4 md:p-6 lg:p-8 space-y-4 md:space-y-6">
                        ...
                        <div className="space-y-3 md:space-y-4">
                           {project.results.map((res, idx) => (
                              <div
                                 key={idx}
                                 className="flex items-center justify-between border-b border-white/5 pb-2 md:pb-3 last:border-0 last:pb-0"
                              >
