    ...
    <div className="fixed right-0 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-4">
      {socials.map((social, index) => (
        <a
          ...
          className={`${bgClass} text-white p-3 pr-4 pl-4 rounded-l-full shadow-lg transform translate-x-2 hover:translate-x-0 transition-transform duration-300 flex items-center justify-center`}
        >
          <social.icon size={20} />
        </a>
      ))}
