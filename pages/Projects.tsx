...
const ProjectsPage: React.FC<ProjectsPageProps> = ({ mode }) => {
  ...
  // Combine projects if 'All' is selected or just filter active mode? 
  // Prompt implies "Filters: All | Editing | Development", so let's combine.
  const allProjects = [...EXTENDED_PROJECTS.video, ...EXTENDED_PROJECTS.dev];
  
  // Use mode to set accent colors
  const isVideo = mode === 'video';
  const accentText = isVideo ? 'text-cine-red' : 'text-blue-500';
  const accentBorder = isVideo ? 'border-cine-red' : 'border-blue-500';
  ...
  const filteredProjects = activeCategory === 'All' 
    ...
    : activeCategory === 'Video' 
      ? EXTENDED_PROJECTS.video 
      : EXTENDED_PROJECTS.dev;
