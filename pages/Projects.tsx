          ...
          <div className="flex bg-white/5 p-1 rounded-full border border-white/5">
            {categories.map(cat => (
              <button
                ...
                className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all ${activeCategory === cat ? 'bg-white text-black shadow-lg' : 'text-slate-400 hover:text-white'
                  }`}
              >
                {cat}
              </button>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            {filteredProjects.map((project: any, idx: number) => (
              ...
              >
                {/* 🔻 Thumbnail (only change done here) */}
                <div className="relative h-44 md:h-52 lg:h-60 rounded-[2rem] overflow-hidden mb-6 border border-white/5 bg-[#111]">
                  <img
                    src={project.image}
                    alt={project.title}
                    loading="lazy"
                  ...
                  <div className="absolute inset-0 flex flex-col justify-center items-center opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0 p-8 text-center">
                    <div className="flex flex-wrap justify-center gap-2 mb-6">
                      {project.tech.map((t: string) => (
                        <span
                          key={t}
                          className="px-3 py-1 bg-white/10 rounded-full text-xs font-bold text-white border border-white/10"
                        >
                          {t}
                        </span>
                      ))}
